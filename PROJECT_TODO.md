...
> **Quy tắc phối hợp:** Mỗi thành viên tick `[x]` vào task của mình khi hoàn thành và commit code kèm mã task (ví dụ: `git commit -m "feat(auth): complete task #2 register API"`).
...
- [ ] **#6 [BE] Spring Security Config & Logout**
  ...
  - *Blocked by:* `#3`, `#5`.
- [ ] **#17 [BE] Public Blog List & Detail APIs**
  ...
  - *Blocked by:* Không có.
- [ ] **#22 [BE] Public Video List & Detail APIs**
  ...
  - *Blocked by:* Không có.
- [ ] **#19 & #24 [BE] Author Blog/Video CRUD & Upload**
...
- [ ] **#26 [FE] Khối Bình luận & Like/Vote**
  ...
  - *File cần tạo:* `frontend/src/components/content/CommentAndVoteSection.jsx`.
...
- [ ] **#27 [BE] API CRUD Bình luận (Comment Engine)**
  - *Mô tả:* Tạo Entity `Comment`, API `POST /api/v1/contents/{id}/comments`, `GET /api/v1/contents/{id}/comments`, `DELETE /api/v1/comments/{id}`.
  - *File cần tạo:* `entity/Comment.java`, `repository/CommentRepository.java`, `service/CommentService.java`.
- [ ] **#28 [BE] API Like/Vote & Chống Trùng lặp**
  - *Mô tả:* Tạo Entity `Vote`, API `POST /api/v1/contents/{id}/vote`, kiểm tra chống vote trùng lặp theo cặp (`user_id`, `content_id`), tự động cập nhật số lượt vote.
  - *File cần tạo:* `entity/Vote.java`, `repository/VoteRepository.java`.
