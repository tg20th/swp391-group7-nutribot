...
export default function ContentCard({ post, onPlay }) {
  ...
  const media = <>{post.image ? <img src={post.image} alt=""/> : <span className="no-media"><FileText size={30}/><span>NutriBot Journal</span><ImageOff size={16}/></span>}{isVideo && <span className="play-overlay"><Play fill="currentColor" size={20}/></span>}</>;
  ...
  return <article className={`content-card ${post.image ? '' : 'content-card--text'}`}>
    ...
    <div className="card-body">
      ...
      <div className="author">{post.avatar ? <img src={post.avatar} alt=""/> : <span className="avatar-fallback">{post.author?.slice(0, 1)}</span>}<span><b>{post.author}</b><small>{[post.username, post.createdAt].filter(Boolean).join(' · ')}</small></span></div>
